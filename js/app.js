...
// Initialize Web Worker (Supports external js/worker.js file or inline Blob fallback)
function initWorker(){
  try {
    ...
    const inlineScript = document.getElementById('workerScript');
    if(inlineScript){
      const blob = new Blob([inlineScript.textContent], {type: 'application/javascript'});
      worker = new Worker(URL.createObjectURL(blob));
      worker.onmessage = function(e){ handleWorkerResult(e.data); };
    }
  }
}
